...
public class VideoTrimmerWorker3 extends ListenableWorker {
    ...
    private void doActualWork(CallbackToFutureAdapter.Completer<Result> completer) {
        ...
        long start = getInputData().getLong("start", 0);
        long end = getInputData().getLong("end", 0);
        TranscoderOptions.Builder transcoder = Transcoder.into(output.getAbsolutePath());
        transcoder.addDataSource((DataSource) new ClipDataSource(VideoUtil.createDataSource(getApplicationContext(), input.getAbsolutePath()), TimeUnit.MILLISECONDS.toMicros(start), TimeUnit.MILLISECONDS.toMicros(end)));
